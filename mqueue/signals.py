...
from mqueue.conf import bcolors, MODELS_CHANNEL
from mqueue.utils import get_user, get_url, get_admin_url, get_object_name


#~ signals
def stream_to_redis(sender, instance, created, **kwargs):
    #~ try to get the user
    ...
    MEvent.objects.create(
                ...
                commit = False,
                stream = True,
                channel = MODELS_CHANNEL,
                )
    return

def stream_to_redis_delete(sender, instance, **kwargs):
    #~ try to get the user
    user = get_user(instance)
